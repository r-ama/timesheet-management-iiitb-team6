...
<s:form action="timesheetapprove" method="POST">
...
<table class="pretty">
<tr>
<th colspan="3"> <h1>Time sheet Approval</h1></th>
</tr>
<tr> 
...
<s:iterator value="day" status="mystat" var="day">
 <td><s:textfield name="timesheet[%{#outerstat.index}].day[%{#mystat.index}]" value="%{day[#mystat.index]}" disabled="true" size="2" cssClass="label" theme="simple"/></td>
 
 	<s:hidden name="timesheet[%{#outerstat.index}].date[%{#mystat.index}]" value="%{date[#mystat.index]}"/>
</s:iterator> 
<td><h3> <s:property value="total"/></h3> </td>
<td><s:checkbox name="timesheet[%{#outerstat.index}].approvalFlag[0]" value="%{approvalFlag[0]}" cssClass="label" theme="simple" /> </td>
...
<s:hidden name="timesheet[%{#outerstat.index}].projectid" value="%{projectid}"/>
 ...
 <s:hidden name="timesheet[%{#outerstat.index}].userid" value="%{userid}"/>
